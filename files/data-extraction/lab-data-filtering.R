...
## isotic soils
iso <- readRDS('data/isotic_pedons_properties.rds')

## mixed soils
mx <- readRDS('data/mixed_pedons_properties.rds')
...
site(x) <- ~ pedlabsampnum + pedoniid + upedonid + longitude_decimal_degrees + latitude_decimal_degrees + samp_name + corr_name + which


## check horizon depth logic
# ~ 2806 affected profiles
...
.v <- c('ph_naf', 'water_15_bar_to_clay_ratio', 'new_zealand_phosphorus_retent', 'aluminum_ammonium_oxalate', 'silica_ammonium_oxalate')
...
ragg::agg_png(filename = 'figures/data-available-example.png', width = 1200, height = 400, scaling = 1.25)
...
# 1638 without aluminum_na_pyro_phosphate
# 106 with aluminum_na_pyro_phosphate
...
a <- slab(
  zz, 
  fm = pedon_key ~ ph_naf + water_15_bar_to_clay_ratio + new_zealand_phosphorus_retent + aluminum_ammonium_oxalate + silica_ammonium_oxalate, 
...
# yikes!
plotSPC(subset(zz, pedon_key == '30683'), color = 'aluminum_ammonium_oxalate', width = 0.1, name.style = 'center-center', cex.names = 0.8)

## TODO: filter these pedons out at top, or at the end
...
## re-attach particle size class data
library("dplyr")
# check that the counts are the same
# count = 1,396
count(unique(iso[c("pedon_key", "corr_taxpartsize")]))
count(unique(iso["pedon_key"]))
# count = 25,364
count(unique(mx[c("pedon_key", "corr_taxpartsize")]))
count(unique(mx["pedon_key"]))

# create new dataframe of just column of interest for unique pedons
iso.u <- unique(iso[c("pedon_key", "corr_taxpartsize")])
mx.u <- unique(mx[c("pedon_key", "corr_taxpartsize")])

# combine iso.u and mx.u
comb.isomx.u <- rbind(iso.u, mx.u)

# bind corr_taxpartsize from "iso" and "mx" to "w"
comb.isomx.u$pedon_key <- as.character(comb.isomx.u$pedon_key)

w <- left_join(w, comb.isomx.u, by = 'pedon_key')
...
## save
saveRDS(w, file = 'data/combined-lab-wt-mean.rds')
