...
vars <- c('ph_naf', 'water_15_bar_to_clay_ratio', 'new_zealand_phosphorus_retent',
          'silica_ammonium_oxalate', 'aluminum_plus_half_iron_oxalat', 'cec_nh4_ph_7')
response <- "isotic"

# subset and shorten names
# .x <- x[, vars]
# names(.x) <- c('pH_NaF', 'ws15_clay', 'NZ_P', 'Si_o', 'Al + 1/2 Fe (AO)', 'CEC7')

# apply backwards elimination on model
library(MASS)

full_model <- glm(isotic ~ ph_naf + water_15_bar_to_clay_ratio + new_zealand_phosphorus_retent +
                    silica_ammonium_oxalate + aluminum_plus_half_iron_oxalat + cec_nh4_ph_7, 
